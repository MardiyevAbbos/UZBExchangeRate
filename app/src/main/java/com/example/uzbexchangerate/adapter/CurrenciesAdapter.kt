...
class CurrenciesAdapter : ListAdapter<ExchangeRate, CurrenciesAdapter.VH>(ITEM_DIFF) {

    private var localData: List<ExchangeRate> = emptyList()
    ...
    inner class VH(private val binding: ItemCurrencyViewBinding) :
        ...
        fun bind(item: ExchangeRate) {
            ...
            if (localData.isNotEmpty() && localData.contains(item)) {
            ...
            binding.ivStar.setOnClickListener {
                if (localData.isNotEmpty() && localData.contains(item)) {
                    binding.ivStar.setImageResource(R.drawable.ic_empty_star)
                    starIconClick?.invoke(item, false)
                } else {
                    binding.ivStar.setImageResource(R.drawable.ic_full_star)
                    starIconClick?.invoke(item, true)
                }
            }
        }
    }
    ...
    fun setLocalData(localItems: List<ExchangeRate>) {
